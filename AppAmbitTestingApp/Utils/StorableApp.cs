...
namespace AppAmbitTestingApp.Utils
{
    public sealed class StorableApp
    {
        ...
        public async Task ClosePreviousSessionIfExists(DateTime endTimestampUtc)
        {
            var tsUtc = DateTime.SpecifyKind(endTimestampUtc, DateTimeKind.Utc);
            ...
            try
            {
                await WithConnection(async db =>
                {
                    ...
                    {
                        Debug.WriteLine("no open session found.");
                        return;
                    }
        ...
        public async Task UpdateLogsWithCurrentSessionId()
        {
            ...
            try
            {
                await WithConnection(async db =>
                {
                    var currentSessionId = await GetCurrentOpenSessionInternal(db).ConfigureAwait(false);
        ...
        public async Task UpdateEventsWithCurrentSessionId()
        {
            ...
            try
            {
                ...
                SET SessionId = ?
                WHERE _rowid_ = (
                    SELECT _rowid_ FROM {EventsTable}
                    ORDER BY _rowid_ DESC
                    LIMIT 1
                );",
                        currentSessionId
